...
use egg::{
    Analysis, BackoffScheduler, CostFunction, EGraph, ENodeOrVar, Extractor, FromOp, Id, Language,
    PatternAst, RecExpr, Report, Rewrite, RewriteScheduler, Runner, SearchMatches, SimpleScheduler,
    StopReason, Subst,
};
...
pub fn detour_run<L: Language, N: Analysis<L> + Default>(
    roots: &[Id],
    rws: &[Rewrite<L, N>],
    eg: &mut EGraph<L, N>,
    hooks: &mut [Hook<L, N>],
...
fn detour_step<L: Language, N: Analysis<L> + Default>(
    i: usize,
    roots: &[Id],
    rws: &[Rewrite<L, N>],
    eg: &mut EGraph<L, N>,
...
fn pat_detour_eqsat_step<L: Language, N: Analysis<L>>(
    ...
    let mut matches: BTreeMap</*detour cost*/ Cost, Vec<(RewriteId, Id, Subst)>> =
        BTreeMap::default();

    for (rw_i, rw) in rws.iter().enumerate() {
        let lhs_pat = rw.searcher.get_pattern_ast().unwrap();

        for m in rw.searcher.search_with_limit(eg, MATCH_LIMIT) {
            // limit to counter oom.
            stopper.check_limits(eg)?;
    ...
    'outer: for (full_cost, new_apps) in matches {
        ...
        for (rw_i, lhs, subst) in &new_apps {
            ...
            if eg_data(eg) != og_data && found_cost.is_none() {
...
fn compute_ctxt_costs<L: Language, N: Analysis<L>>(
    roots: &[Id],
    eg: &EGraph<L, N>,
    ex: &Extractor<AdditiveCostFn<L>, L, N>,
    cf: fn(&L) -> Cost,
) -> HashMap<Id, Cost> {
...
impl Stopper {
    fn check_limits<L: Language, N: Analysis<L>>(
        &self,
        eg: &EGraph<L, N>,
    ) -> Result<(), StopReason> {
